  ...
  return (
    <TooltipPrimitive.Provider delayDuration={50}>
      <TooltipPrimitive.Root
        open={isTouchDevice.current ? open : undefined}
        onOpenChange={(next) => {
          if (!isTouchDevice.current) setOpen(next);
        }}
      >
        <TooltipPrimitive.Trigger asChild>
          <div
            className={cn("relative inline-flex text-center", className)}
            onPointerDown={(e) => {
              if (isTouchDevice.current || e.pointerType === "touch") {
                e.preventDefault();
                setOpen((prev) => !prev);
              }
            }}
            aria-label="Show tooltip"
            aria-expanded={isTouchDevice.current ? open : undefined}
          >
            {icon}
          </div>
